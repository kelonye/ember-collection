var layout = Em.HTMLBars.template(require('./template'));
var identity = require('../utils/identity');
var needsRevalidate = require('../utils/needs-revalidate');
var get = Ember.get;
var set = Ember.set;

function Cell(key, item, index, style) {
  if (!(this instanceof Cell)) return Cell(key, item, index, style);
...
module.exports = Ember.Component.extend({
  ...
  updateCells: function updateCells() {
    ...
    for (i = 0; i < newItems.length; i++) {
      ...
      var _item = items.objectAt(itemIndex);
      itemKey = identity(_item);
      style = this._cellLayout.formatItemStyle(itemIndex, this._clientWidth, this._clientHeight);
      cell = new Cell(itemKey, _item, itemIndex, style);
      cellMap[itemKey] = cell;
      this._cells.pushObject(cell);
    }
    this._cellMap = cellMap;
  },
  _isGlimmer2: function _isGlimmer2() {
